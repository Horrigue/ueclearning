...
AMyBullet::AMyBullet()
{
	...
	MyShpere = CreateDefaultSubobject<USphereComponent>(TEXT("MySphereComponent"));
	MyProjectile = CreateDefaultSubobject<UProjectileMovementComponent>(TEXT("MyProjectileComponent"));
	static ConstructorHelpers::FObjectFinder<UStaticMesh>TmpStaticMesh(TEXT("/Script/Engine.StaticMesh'/Game/StarterContent/Shapes/Shape_Cube.Shape_Cube'"));
	MyBullet ->SetStaticMesh(TmpStaticMesh.Object);
	MyBullet ->SetRelativeScale3D(FVector(0.4, 0.4, 0.4));
	RootComponent = MyBullet;
	MyShpere ->SetupAttachment(MyBullet);
